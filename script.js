...
const WEIGHT_HEADERS = ["вес", "граммы", "вес/объем", "вес / объем", "weight"];
...
const BASE_PRODUCTS = [
  ...
  product("vareniki-tvorog", "Вареники с творогом", 400, "400 ₽", "hot", "in", "images/Вареники с творогом 400.webp"),
  product("galushki", "Галушки", 220, "220 ₽", "hot", "in", "images/Галушки 220р.webp"),
  product("golubtsy", "Голубцы 10 шт", 1000, "1000 ₽", "hot", "in", "images/Голубцы 10шт 1000р.webp"),
  product("zazharka", "Зажарка", 190, "190 ₽", "extra", "in", "images/Зажарка 190.webp"),
  product("manty-squares", "Квадратики на манты 50 шт", 300, "300 ₽", "semi", "in", "images/Квадратики на манты 50шт 300р.webp"),
  product("kiev-cutlets", "Котлеты по-киевски 5 шт", 700, "700 ₽", "meat", "preorder", "images/Котлеты по - киевски 5шт 700р.webp"),
  product("chicken-cutlets", "Куриные котлеты 10 шт", 850, "850 ₽", "meat", "in", "images/Куриные котлеты 10шт 850.webp"),
  product("chicken-chops", "Куриные отбивные 5 шт", 600, "600 ₽", "meat", "in", "images/Куриные отбивные 5шт 600р.webp"),
  ...
  product("meat-cutlets", "Мясные котлеты 10 шт", 850, "850 ₽", "meat", "in", "images/Мясные котлеты 10шт 850.webp"),
  product("meat-chicken-rolls", "Мясные и куриные рулеты", 2000, "2000 ₽", "meat", "preorder", "images/Мясные, куриные рулеты 2200.webp"),
  product("nut-cake", "Ореховый торт", 1300, "1300 ₽", "desserts", "preorder", "images/Ореховый торт 1300.webp"),
  product("pelmeni", "Пельмени", 450, "450 ₽", "hot", "in", "images/Пельмени 450.webp"),
  product("liver-cake", "Печеночный торт", 1600, "1600 ₽ / 850 ₽", "desserts", "in", "images/Печеночный торт - 1600 800.webp"),
  product("cleaned-chicken", "Разделанная очищенная курица", 950, "950 ₽", "meat", "preorder", "images/Разделанная, очищенная курица 1т.webp"),
  ...
  product("syrniki", "Сырники 10 шт", 1000, "1000 ₽", "desserts", "in", "images/Сырники 10шт 550.webp"),
  product("tefteli", "Тефтели 10 шт", 900, "900 ₽", "meat", "in", "images/Тефтели 10шт 900р.webp"),
...
function product(id, name, price, priceText, category, status, image, description = "", oldPrice = 0, oldPriceText = "", weight = "") {
  return {
    id,
    name,
    price,
    ...
    oldPrice,
    oldPriceText,
    variants: id === "strawberry-jam" ? STRAWBERRY_VARIANTS : (CAKE_VARIANTS[id] || [])
  };
}
...
function variantsFromPrices(baseVariants, wholePrice, halfPriceCell, productId = "") {
...
function productsFromRows(rows) {
  ...
  return rows.map((row, index) => {
    ...
    const weightText = rowValue(row, WEIGHT_HEADERS);
    ...
    if (base) {
      return {
        ...
        variants: variantsFromPrices(base.variants, priceData.price, halfPriceText, base.id)
      };
    }
...
function applyCategoryFilter(categoryId, behavior = "smooth") {
  ...
  catalog.querySelectorAll(".category-section").forEach((section) => {
    ...
    const hiddenForAll = categoryId === "all" && sectionCategory === "available";
    const hiddenForCategory = categoryId !== "all" && sectionCategory !== categoryId;
    section.hidden = hiddenForAll || hiddenForCategory;
  });
